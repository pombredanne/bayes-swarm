...
module Pulsar
  ...
  # A simple definition for a stem, that tracks its name, the number of occurrences
  # within a given domain (e.g. a webpage) and its id (to link it back to the database)
  class StemData
  ...
  class CustomLists
    ...
    # Returns a customized list of stopwords. This list is generated by removing 
    # a custom list of stopwords (as read from the configuration file under the +keep_stopwords+ key)
    # from the +ferret_stopwords+ list.
    def custom_stopwords
      stopwords_to_keep = Pulsar::Runner.opts['keep_stopwords']

      result = {}
      @langs.each do |lang|
        result[lang] = ferret_stopwords[lang].reject { |word| stopwords_to_keep[lang].include?(word)}
      end
  ...
  # Performs stemming and stopwords removal from a +content+. Operates in multiple languages.
  class FerretStemmer
    def initialize    
  ...
  # The stemming and stopwords filter internally used by +FerretStemer+. *NOTE* that it does not apply stemming
  # to the italian language. It has been found in the past that results are better this way.
  class StopAndStemAnalyzer < Ferret::Analysis::Analyzer
    include Ferret::Analysis

    def token_stream(field, str, lang, stopwords)
      if lang == :it
        return StopFilter.new(LowerCaseFilter.new(StandardTokenizer.new(str)), stopwords)
      else
  ...
  class StemAggregator
    ...
    # 
    # +stems+ is an array of stems to be filtered. 
    ...
    def interesting_filter(stems, intwords_hash)
      stemdata_hash = to_stem_hash(stems)
      int_stems_found = Array.new

      stemdata_hash.each do |stem,stemdata|
        ...
        if intwords_hash.has_key?(stem)
          ...
          stemdata_hash.delete(stem)
        end
      end
      int_stems_found = int_stems_found.sort_by { |stemdata| stemdata.count }.reverse
    ...
    #
    # +stems+ is an array of stems to be filtered, +threshold+ is a number representing
    # the minimum number of stem occurrences to qualify it as popular.
    def popularize(stems, threshold)
      popular_stems_found = to_stem_hash(stems).values
      
      # Applies various filters to prune irrelevant stems.
      popular_stems_found = popular_stems_found.
        select { |stemdata| stemdata.stem.length > 2 }. # Filters out stems that are too short
        select { |stemdata| stemdata.stem !~ /\d+/ }. # Filters out numbers
        select { |stemdata| stemdata.count > threshold } # Filters out stems that are below the threshold 
        
      popular_stems_found.sort_by { |stemdata| stemdata.count }.reverse
      return popular_stems_found
    end
        
    # Converts a list of stems into an hash that associate each stem to a +StemData+ structure.
    def to_stem_hash(stems)
      stem_hash = Hash.new
      stems.each do |stem|
