...
query <- dbSendQuery(mycon, paste("select date(scantime) as date, s.id as source_id, page_id, intword_id, count
                                   ...
                                   where w.page_id=p.id 
                                         ...
                                         and w.intword_id in (3, 200, 67103, 1, 11238, 4323, 885, 62, 16114, 27, 5512, 13360, 25, 2831, 45, 24, 11106, 21, 69826, 65732, 69019, 65195, 4, 12600, 91, 5132, 49113, 42, 2376, 300, 66920, 65343, 9958, 71786, 65595, 2576, 65145, 321, 335, 336, 9302, 14659, 3579, 330, 51151, 18288, 8, 5248, 32532, 67164, 51712", stopwords_ids_list,")
                                         and date(scantime) between '2008-08-01' and '2008-11-30';"))
data <- fetch(query, n = -1)

query <- dbSendQuery(mycon, paste("select id, name
                                   from intwords
                                   where id in (3, 200, 67103, 1, 11238, 4323, 885, 62, 16114, 27, 5512, 13360, 25, 2831, 45, 24, 11106, 21, 69826, 65732, 69019, 65195, 4, 12600, 91, 5132, 49113, 42, 2376, 300, 66920, 65343, 9958, 71786, 65595, 2576, 65145, 321, 335, 336, 9302, 14659, 3579, 330, 51151, 18288, 8, 5248, 32532, 67164, 51712", stopwords_ids_list,");"))
