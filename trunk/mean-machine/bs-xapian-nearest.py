...
class Filter(xapian.ExpandDecider):
    def __init__(self, excluded_terms, stopwords):
...
eset = enquire.get_eset(50, rset, Filter([term], stopwords[lang]))

tagscores = dict()
print "term, term2, weight, distance"
for item in eset:
    #print item.term
    ...
    for y, m in enumerate(mset):
        ...
        try:
            s1 = set(db.positionlist(docid, term))
            s2 = set(db.positionlist(docid, item.term))
            count = []
            for i in s1:
                for j in s2:
                    count.append(abs(i-j))
            mins.append(min(count))
        except xapian.RangeError:
            pass
    if mins != []:
        #print term, ",", item.term, ",", item.weight, ",", sum(mins)/float(len(mins))
        print ",".join([term, item.term, "%f" % item.weight, "%f" % (sum(mins)/float(len(mins)))])
