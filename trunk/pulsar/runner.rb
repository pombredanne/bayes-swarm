...
module Pulsar
  ...
  # The Runner is the entry point for execution. It is responsible for loading configuration
  # options and firing execution of user defined scripts (or _components_ ).
  class Runner
    include Pulsar::Log
    
    # Creates a new instance of the class, using +config_file+ as the source of configuration options
    ...
    # Executes the script +script_file+ by +require+ -ing it. The script should be created in such
    # a way to start executing when required, pretty much like when launched from the
    # command line
    def go(script_file)
    ...
    # Returns the options loaded from the config file. This method it is created for general
    # consumption from everywhere in the code.
    def Runner.opts
      @@opts
    end
