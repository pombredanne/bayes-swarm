...
# Extends the default PageStore to provide additional information in the +META+ 
# file pertaining to the +Page+ the stored entry belongs to (such as id, kind 
# and language code).
#
# == Author
...
module Pulsar
  ...
  class BayesPageStore < Pulsar::PageStore
    ...
    def get_meta
      return super + 
             (@page ? 
                " #{@page.id} #{@page.kind_name} #{@page.language_name}" : 
                "" )
    end
